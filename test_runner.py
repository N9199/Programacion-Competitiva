## Taken from Pablo Messina's Repo, with permission
import subprocess
import sys
import tqdm
...
for _ in tqdm.trange(n):
    test = subprocess.run(
        generator_command.split(), stdout=subprocess.PIPE).stdout
    canonical_answer = subprocess.run(
        canonical_command.split(), input=test, stdout=subprocess.PIPE)
    main_answer = subprocess.run(
        main_command.split(), input=test, stdout=subprocess.PIPE)
